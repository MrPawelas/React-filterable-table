...
class App extends React.Component {
  ...
  onNew(model) {    
    ...
    let lastNewAddedCounter = this.state.lastNewAdded // Names are keys in table so they have to be unique. Thanks to that if a user clicks multiple times add new it doesnt crash.
    
    if(this.state.models.filter(miniature => miniature[0] === nameModel)){
      console.log(this.state.models.filter(miniature => miniature[0] === nameModel))
