...
export class FilterAndSortComponent extends React.Component {
    ...
    createFilter() {
        ...
        if (this.state.ratingDownFilter) {
            ...
            result[1] = (model) => { return (model[3] >= parseInt(this.state.ratingDownFilter)) }
        }
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (prevState.nameFilter !== this.state.nameFilter ||
             prevState.descriptionFilter !== this.state.descriptionFilter ||
              prevState.ratingDownFilter !== this.state.ratingDownFilter ||
               prevState.ratingUpperFilter !== this.state.ratingUpperFilter) {
        ...
        return (
            <Container fluid >
                <Row>
                    ...
                    <Col className="text-start">Name: <input type="text" name="nameFilter" value={this.state.nameFilter} onChange={this.onFilterInputChange} /></Col>
                    <Col className="text-start">Description: <input type="text" name="descriptionFilter" value={this.state.descriptionFilter} onChange={this.onFilterInputChange} /></Col>
