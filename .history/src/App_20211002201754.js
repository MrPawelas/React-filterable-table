...
class App extends React.Component {
  ...
  onNew(model) {    
    ...
    let lastNewAddedCounter = this.state.models.filter(miniature => miniature[0] === nameModel).length // Names are keys in table so they have to be unique. Thanks to that if a user clicks multiple times add new it doesnt crash.
    
    if(lastNewAddedCounter>0){
      nameModel = nameModel+(lastNewAddedCounter+1)
    }
