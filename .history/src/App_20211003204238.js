...
class App extends React.Component {
  ...
  onRatingChange(miniatureName, ratingChange) {
    const updatedModels = this.state.models.map(miniature => {
      if (miniature[0] === miniatureName && miniature[3]+ratingChange >= 0 && miniature[3]+ratingChange <= 10) {
        miniature[3] += ratingChange
      }
      return miniature
    })
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        <TableComponent models={visbleModels} onDelete={this.onDelete} onRatingChange={this.onRatingChange} />
      </div>
    );
  }
}
